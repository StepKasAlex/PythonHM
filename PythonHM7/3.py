...
# (add()), вычитание (sub()), умножение (mul()), деление (truediv()).Данные методы должны 
...
class Cell():
	...
	def __sub__(self, other):
		if self.number_of_cells - other.number_of_cells > 0:
			...
			return 'Difference 0 or less than 0'

	def __mul__(self, other):
		return self.number_of_cells * other.number_of_cells

	def __truediv__(self, other):
		return round(self.number_of_cells / other.number_of_cells)

	def make_order(self, count = 3):
		list_of_cells = []
		for i in list(range(1, self.number_of_cells + 1)):
			print(i, end=' ')
			list_of_cells.append(i)
			if len(list_of_cells) == count:
				print()
				list_of_cells.clear()
